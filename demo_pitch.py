...
pre_pitch = 0
beat_flag = 0
def get_hole_pitch(input_pitch, input_confidence):
    global pre_pitch
    global same_pitch_count
    global music_pitch
    global beat_flag
    ...
    if input_confidence > 0.8:
        if abs(input_pitch - pre_pitch) > 0.5:
            print("abs: " + str(abs(input_pitch - pre_pitch)) + '\n')
            beat_flag = 0
            same_pitch_count = 1
            pre_pitch = input_pitch
        elif beat_flag == 0:
            pre_pitch = input_pitch
            same_pitch_count += 1
    else:
    ...
    if same_pitch_count >= 5:
        same_pitch_count = 0
        music_pitch.append(input_pitch)
        beat_flag = 1
...
while True:
    ...
    get_hole_pitch(pitch, confidence)
    pitches += [pitch]
    confidences += [confidence]
    total_frames += read
    if read < hop_s: break

print("music midi: " + str(music_pitch) + '\n')
...
from waveform_plot import get_waveform_plot, set_xlabels_sample2time
